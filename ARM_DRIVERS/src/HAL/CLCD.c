...
/**************************Global Variables*****************************/
static u32 CLCD_EnablePinState = CLCD_ENABLE_OFF;
...
static CLCD_enuErrorStatus_t CLCD_SendCommand(u8 Command);
static CLCD_enuErrorStatus_t CLCD_SendData(u8 Data);
static CLCD_enuErrorStatus_t CLCD_CntrlEnablePin(u32 EnablePins_State);
static CLCD_enuErrorStatus_t CLCD_CLEAR(void);
static CLCD_enuErrorStatus_t CLCD_SetPosition_Helper(u8 *Ptr_PositionDDRAM);
/************************Process functions******************************/
...
static CLCD_enuErrorStatus_t CLCD_PowerOn_proc(void)
{
	...
	u8 Loc_index;
	GPIO_PinConfig_t clcd_pin;
	clcd_pin.Mode=GPIO_OUTPUT_PP;
	clcd_pin.Speed=GPIO_HIGH_SPEED;
...
static CLCD_enuErrorStatus_t CLCD_SendCommand(u8 Command)
{
    ...
    if (CLCD_EnablePinState== CLCD_ENABLE_OFF)
 {
    ...
    for (Loc_index=0;Loc_index<_LCD_NUM-CONTROL_PINS;Loc_index++)
    {
    	...
    	Ret_enuErrorStatusCLCD=GPIO_Set_PinValue(LCDS[Loc_index].Port,LCDS[Loc_index].Pin,Loc_Buffer);
...
static CLCD_enuErrorStatus_t CLCD_CntrlEnablePin(u32 EnablePins_State)
{
	...
	Ret_enuErrorStatusCLCD= GPIO_Set_PinValue(LCDS[EN].Port,LCDS[EN].Pin,EnablePins_State);
    return Ret_enuErrorStatusCLCD;
}
